...
void src_file_check(const char* f, const std::vector<CheckSumInfo>& info)
{
	...
	while (cur_idx < len)
	{
		...
		{
			sum = rolling_checkSum(last_sum, file_data[cur_idx - 1], file_data[cur_idx + block_sz], block_sz); 
			test_len = block_sz; 
		}
		else
		{
	...
	for (size_t i = 0; i != vec.size(); ++i)
	{
		if (vec[i].type == 1)
		{
			printf("send:%d type:%d id:%d", i, vec[i].type, vec[i].len); 
		}
		else
		{
			std::string w(vec[i].data.begin(), vec[i].data.end()); 
			printf("send:%d type:%d data:%s", i, vec[i].type, w.c_str()); 
		}
	}
}
